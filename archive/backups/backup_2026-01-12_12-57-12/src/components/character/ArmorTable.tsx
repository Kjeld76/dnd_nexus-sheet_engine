...
export const ArmorTable: React.FC<Props> = ({ character, armor }) => {
  const { updateInventory, saveCharacter } = useCharacterStore();
  ...
  const handleToggleEquip = (armorId: string) => {
    ...
    if (isCurrentlyEquipped) {
      ...
      if (!isShield) {
        ...
        const otherEquippedArmor = character.inventory
          .map((invItem) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {showAddDialog && (
        <div className="bg-muted/30 p-4 rounded-xl border border-border space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar">
          {availableArmor.length === 0 ? (
            ...
            availableArmor.map((armorItem) => (
              <button
                ...
                className="w-full text-left bg-card p-3 rounded-lg border border-border hover:border-primary/30 hover:bg-primary/5 transition-all group"
              >
      ...
      <div className="space-y-2 max-h-[600px] overflow-y-auto custom-scrollbar">
        {inventoryArmor.length === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-[1fr_auto] gap-4 px-2 pb-2 border-b border-border/50">
            ...
            {inventoryArmor.map(({ armor: armorItem, ...invItem }) => {
              ...
              return (
                <div
                  key={invItem.id}
                  className={`bg-card p-4 rounded-xl border transition-all group hover:border-primary/30 ${
                    isEquipped
