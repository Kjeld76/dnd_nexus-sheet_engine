...
function optimizeBackgroundEquipment() {
  console.log('🔧 Optimiere starting_equipment Items...\n');
  
  // Verarbeite core_backgrounds
    ...
    for (const option of data.starting_equipment.options) {
      ...
      for (const item of option.items) {
        if (typeof item === 'string') {
          ...
          // Bereits strukturiert, aber prüfe ob es korrekt ist
          if (!item.name || item.name.trim() === '') {
            ...
            if (item.unit && item.quantity) {
              ...
              bgChanges.push(
                `  ⚠️  Fehlerhafter Eintrag (übersprungen): ${JSON.stringify(item)}`
              );
              continue;
            }
          } else if (item.variant && item.variant.includes('drei Flaschen')) {
            // Spezialfall: "Öl (drei Flaschen)" wurde falsch geparst
            const fixed = {
              name: item.name,
              quantity: 3,
              unit: 'Flaschen',
  ...
  // Speichere Report
  fs.writeFileSync(
    './background-equipment-optimization-report.json',
    JSON.stringify(changes, null, 2)
  );
  console.log('💾 Report gespeichert: background-equipment-optimization-report.json');
}

console.log('═'.repeat(80));
console.log('D&D NEXUS - BACKGROUND EQUIPMENT OPTIMIERUNG');
console.log('═'.repeat(80) + '\n');

try {
  optimizeBackgroundEquipment();
  console.log('═'.repeat(80) + '\n');
} catch (error) {
  console.error('\n❌ FEHLER:', error);
  process.exit(1);
