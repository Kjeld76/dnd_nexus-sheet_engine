...
fn parse_properties(props_str: &str) -> Vec<(String, Option<String>)> {
    ...
    for part in parts {
        ...
        // Vielseitig (1W8)
        if let Some(caps) = regex::Regex::new(r"Vielseitig\s*\((\d+W\d+)\)")
            .unwrap()
            .captures(part)
        {
        ...
        if let Some(caps) = regex::Regex::new(r"Wurfwaffe\s*\(Reichweite\s*(\d+(?:,\d+)?)\/(\d+(?:,\d+)?)\)")
            .unwrap()
            .captures(part)
        {
        ...
        if let Some(caps) = regex::Regex::new(r"Geschosse\s*\(Reichweite\s*(\d+(?:,\d+)?)\/(\d+(?:,\d+)?)(?:,\s*([^)]+))?\)")
            .unwrap()
            .captures(part)
        {
            let normal = caps.get(1).unwrap().as_str().replace(',', ".").parse::<f64>().unwrap_or(0.0);
