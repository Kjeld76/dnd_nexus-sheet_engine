...
export const SpeciesTraits: React.FC<Props> = ({ species }) => {
  ...
  return (
    <div className="bg-card p-6 rounded-[3.5rem] border border-border shadow-2xl shadow-foreground/[0.02]">
      ...
      <div className="grid grid-cols-1 gap-6">
        {traits.map((trait: any) => {
          ...
          return (
            <div
              key={trait.name}
              className="bg-background p-6 rounded-[2.5rem] border border-border hover:border-primary/40 transition-all group relative overflow-hidden"
            >
