...
export const SkillList: React.FC<Props> = ({ character }) => {
  ...
  const calculateBonus = (skill: typeof SKILLS[0]) => {
    const isProficient = character.proficiencies?.skills?.includes(skill.name) ?? false;
    const attrMod = stats.saving_throws[skill.attr as keyof typeof stats.saving_throws]; // base mod
      ...
      <div className="flex items-center justify-between mb-10 border-b border-border pb-8">
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
            <h2 className="text-3xl font-black tracking-tighter italic font-serif">Fertigkeiten</h2>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground opacity-50">Proficiency & Expertise</p>
          </div>
        </div>
            ...
            >
              <div className="flex items-center gap-5">
                ...
                <div className="flex flex-col -space-y-0.5">
                  <span className="text-base font-bold tracking-tight text-foreground group-hover:text-primary transition-colors">{skill.name}</span>
                  <span className="text-[10px] text-muted-foreground uppercase font-black tracking-widest opacity-40 group-hover:opacity-60">({skill.attr})</span>
                </div>
              </div>
